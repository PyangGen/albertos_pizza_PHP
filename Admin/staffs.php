...
$sql = "SELECT * FROM staff LIMIT $itemsPerPage OFFSET $offset"; // Add LIMIT and OFFSET for pagination

// Get total users count for pagination
$totalResult = $conn->query("SELECT COUNT(*) AS count FROM staff");
    ...
    <tbody>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $formattedDate = date('F j, Y', strtotime($row['createdAt'])); // format date here
                echo "<tr>
